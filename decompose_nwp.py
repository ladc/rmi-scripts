...
if __name__ == "__main__":
    ...
    for fname in fnames:
        if not os.path.exists(fname):
            logging.error(
                "file %s does not exist, no output has been generated. Moving on the the next file",
                fname,
            )
            erno = erno + 1
            erno = erno / erno
            ...
            r_nwp, metadata_nwp = transformer(
                r_nwp, metadata_nwp, threshold=metadata_nwp["threshold"]
            )
            ...
            filter_cascade = pysteps.cascade.bandpass_filters.filter_gaussian(
                r_nwp.shape[1:], ncascades
            )
